# URE — Universal Render Engine v1.2.0
...
//     unstable: 20,
//     preCached: 6,       // items ที่ pre-render ไว้ใน idle cache
//     mountCap: 8,        // max DOM mounts ต่อ rAF frame สำหรับ buffer zone
//     horizontal: false,
...
- **Two-tier mounting** *(v1.2.0)* — viewport items (ที่เห็นได้จริง) mount ทั้งหมดทันทีไม่จำกัด cap เพื่อป้องกัน gap; buffer items ยังคง cap ตาม device tier เพื่อ frame budget
- **Partial fast-scroll correction** *(v1.2.0)* — `_applyCorrection()` rebuild offsets + update transforms ทุก velocity (ป้องกัน overlap); scrollBy anchor correction ถูก skip เฉพาะตอน velocity สูง (ป้องกัน fight scroll momentum)
- **Snap-correct on scroll-end** *(v1.2.0)* — flush pending corrections ทันทีเมื่อ scroll หยุด ไม่ต้องรอ timer หมด — ป้องกัน content กระตุกหลังหยุดเลื่อน
- **Velocity buffer** — extend buffer ในทิศที่กำลัง scroll, shrink buffer ด้านหลัง เพื่อลด DOM node ที่ active
- **rAF gating + mount cap** — DOM mount buffer zone ไม่เกิน `_MOUNT_CAP` nodes ต่อ frame (4/8/16 ตาม device tier); viewport zone ไม่จำกัด
...
| Tier | Cores | Memory | Render chunk | Preload chunk | Buffer mount cap |
|---|---|---|---|---|---|
| 0 (low-end)   | ≤ 2  | ≤ 1 GB | 4  | 8  | 4  |
| 1 (mid-range) | ≤ 4  | ≤ 2 GB | 8  | 16 | 8  |
| 2 (high-end)  | > 4  | > 2 GB | 16 | 32 | 16 |

> **หมายเหตุ v1.2.0:** mount cap ใช้กับ buffer zone เท่านั้น — items ใน viewport จริงไม่มีการจำกัด cap ในทุก tier
...
**`virtual-list.js` only**

**Bug fixes:**
- **[FIX] Gaps during fast scroll** — viewport items now always mount immediately with no cap. Buffer-zone items keep the per-frame cap. Eliminates blank gaps when flinging to a new position.
- **[FIX] Overlapping elements during fast scroll** — `_applyCorrection()` now always rebuilds the offset array and updates visible item transforms at any scroll velocity. Only the `scrollBy()` scroll-anchor step is skipped at high velocity (that's what caused scroll-fighting). Eliminates items rendering on top of each other.
- **[FIX] Content snap/jump after scroll stops** — pending height corrections are now flushed immediately when the scroll-idle timer fires instead of waiting for the next `_corrTimer` delay. Eliminates the "content snaps into place" artifact after stopping.

**Internal improvements:**
- Extracted `_mountNode()` helper — shared between viewport-tier and buffer-tier mount loops, no code duplication.
- `stats()` now exposes `preCached` count for easier debugging of the pre-render cache.

### v1.1.0 — Horizontal Mode + Scroll-Guard
- Added `horizontal` option (axis-aware virtual scroll with `translateX`)
- Added `_coOffPending` scroll-guard to prevent `getBoundingClientRect` during nav bar animations
- Added `_rebuildFrom(i)` partial offset rebuild

### v1.0.0 — Initial Release
- Virtual scroll with Float64Array prefix sums
- DOM node recycling pool
- 2-pass O(n+m) diffing engine
- Web Worker bridge (filter / sort / paginate)
- Lazy asset loading with CLS prevention

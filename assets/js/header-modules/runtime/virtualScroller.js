// virtualScroller.js
// Small windowing helper: render only visible items using a renderer callback.
// Usage:
//   const vs = createVirtualScroller(container, { itemHeightEstimate, buffer: 3, totalItems, renderRange })
//   vs.updateTotal(newTotal)
//   vs.scrollToIndex(i)
...
export function createVirtualScroller(container, options = {}) {
  ...
  function doRender() {
    ...
    renderedStart = start;
    renderedEnd = end;
    // call renderRange(start, end)
    try { renderRange(start, end); } catch (e) { console.error(e); }
  }

  function updateTotal(n) { total = n; scheduleRender(); }
  function scrollToIndex(i) { container.scrollTop = Math.max(0, i * itemHeightEstimate); scheduleRender(); }

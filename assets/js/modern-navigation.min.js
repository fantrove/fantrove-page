 ...
 *
 * Adjustments in this build:
 * - Integrates with the page-level language system (languageManager) when available:
 *   - Prefers languageManager.selectedLang as currentLang
 *   - Calls languageManager.recordLanguageForPath when intercepting navigation (for popstate prediction)
 *   - Calls languageManager.updatePageLanguage when a candidate with lang prefix is found
 * - Ensures clear local/dev vs production behavior:
 *   - On local/dev hosts (ModernNavigation._isLocalDev()), the generator will NOT prefix nav URLs and aggressive candidate probing is disabled.
 *   - On production hosts, full aggressive language-aware navigation is enabled.
 */
...
class ModernNavigation {
  constructor(config = {}) {
    ...
    this.defaultLang = 'en';
    // Prefer languageManager selectedLang when available
    this.currentLang = (window && window.languageManager && window.languageManager.selectedLang) || localStorage.getItem('selectedLang') || this.defaultLang;
    ...
    // Option: บังคับเติม language prefix เสมอเมื่อรู้ภาษาที่เลือก (default: true)
    this.forceLangPrefix = (typeof config.forceLangPrefix !== 'undefined') ? !!config.forceLangPrefix : true;

    // Detect once if we're running in local/dev environment
    this._isLocalDevEnv = ModernNavigation._isLocalDev();
  ...
  _generateNavUrl(baseUrl) {
    ...
    // If running in local/dev environment, do NOT add language prefixes (developer-friendly)
  ...
  async _tryNavigateCandidates(destinationPath, originalEvent) {
    // If local dev, skip aggressive tries to avoid filesystem/server odd errors during dev
    if (this._isLocalDevEnv) return false;
  ...
  _createNavigationItemsFragment(){
    ...
    for (let i = 0; i < config.length; i++){
      ...
      // If local dev environment, do NOT prefix — keep dev-friendly URLs
      if (this._isLocalDevEnv) {
        link.href = baseUrl.startsWith('/') ? baseUrl : '/' + baseUrl;
      } else {
  ...
  _bindBaseListeners(){
    ...
    // NOTE: we now enforce a strict reload on popstate when language implied by URL/storage differs from current language (on non-local hosts).
    window.addEventListener('popstate', (ev) => {
      try {
        ...
        // If local dev, avoid forcing reload and instead attempt soft sync via languageManager (if available)
        if (this._isLocalDevEnv) {
          // Try to let languageManager predict/update language for this location
          try {
            if (window.languageManager && typeof window.languageManager.getPredictedLangForPath === 'function') {
              const predicted = window.languageManager.getPredictedLangForPath(window.location.pathname + (window.location.search||''));
              if (predicted && predicted !== (window.languageManager.selectedLang || current)) {
                window.languageManager.updatePageLanguage(predicted).catch(()=>{});
              }
            }
          } catch (e) {}
          // simply update active state
          this._updateActiveState();
          return;
        }

        // production behaviour: if mismatch between URL lang/state version and storedVersion -> reload
        if ((stateVersion !== null && stateVersion !== storedVersion) || (langInUrl && langInUrl !== current)) {
          if (!ModernNavigation._isLocalDev()) {
            try {
              // If languageManager is present, try to update language first (non-blocking) then reload to ensure full server-rendered prefixed page
              if (window.languageManager && typeof window.languageManager.updatePageLanguage === 'function' && langInUrl) {
                try { window.languageManager.updatePageLanguage(langInUrl).catch(()=>{}); } catch(e){}
              }
  ...
  _unbindBaseListeners(){
    ...
    window.removeEventListener('popstate', () => {}); // no-op
  ...
  _onNavClick(event){
    ...
    // Before navigating, record mapping path -> intended language so popstate prediction can use it
    try {
      const intendedLang = (window && window.languageManager && window.languageManager.selectedLang) || this.currentLang || null;
      this._recordMappingForDestination(destination, intendedLang, 'nav-click');
    } catch (e) {}
    ...
    // If local dev env -> skip aggressive probing and go straight to generated href
    if (this._isLocalDevEnv) {
      try {
        ...
        if (linkEl) {
          ...
          // fallback to destination (no prefix)
          const dest = destination.startsWith('/') ? destination : '/' + destination;
          window.location.assign(dest);
          return;
        }
      } catch (e) {
        try { location.href = destination; } catch(e){}
        return;
      }
    }
  ...
  /* --- record mapping utility --- */
  _recordMappingForDestination(dest, lang, evidence = 'nav-click') {
    try {
      // Prefer languageManager.recordLanguageForPath if available
      if (window.languageManager && typeof window.languageManager.recordLanguageForPath === 'function') {
        try {
          window.languageManager.recordLanguageForPath(dest, lang);
          return;
        } catch (e) {}
      }
      // Otherwise write directly to sessionStorage map (fv-nav-lang-map)
      const tmp = new URL(dest, location.origin);
      const key = tmp.pathname + (tmp.search || '');
      const rawMap = sessionStorage.getItem('fv-nav-lang-map') || '{}';
      const map = JSON.parse(rawMap || '{}');
      map[key] = { lang: lang, ts: Date.now(), evidence: evidence };
  ...
  _updateAllNavUrls(){
    ...
    for (let i = 0; i < items.length; i++){
      ...
      // If local dev env, do NOT prefix
      if (this._isLocalDevEnv) {
        item.href = baseUrl.startsWith('/') ? baseUrl : '/' + baseUrl;
        continue;
      }

...
class ModernNavigation {
  ...
  /**
   * สร้าง URL สำหรับ navigation link โดยอัตโนมัติเติม language prefix ตาม current URL
  ...
  _generateNavUrl(baseUrl) {
    ...
    // ถ้า current URL ไม่มี language prefix ให้ใช้ baseUrl ตามเดิม
    if (!parsed.hasLangPrefix) {
      return baseUrl;
    }
    
    // ถ้า baseUrl เป็น external link (เริ่มต้นด้วย http หรือ //) ให้ใช้ตามเดิม
    if (baseUrl.startsWith('http') || baseUrl.startsWith('//')) {
      return baseUrl;
    }
    
    // ถ้า baseUrl เป็น hash link ให้ใช้ตามเดิม
    if (baseUrl.startsWith('#')) {
      return baseUrl;
    }
    
    // ถ้า baseUrl มี language prefix อยู่แล้ว ให้ใช้ตามเดิม
    const baseSegments = baseUrl.split('/').filter(s => s.length > 0);
    if (baseSegments.length > 0 && this._isLangCode(baseSegments[0])) {
      return baseUrl;
    }
    
    // เติม language prefix เข้าไป
    const normalizedBase = baseUrl.startsWith('/') ? baseUrl : '/' + baseUrl;
    return '/' + parsed.lang + normalizedBase;
  }
  ...
  _createNavigationItemsFragment(){
    ...
    for (let i = 0; i < config.length; i++){
      ...
      // ✅ ใช้ URL ที่ปรับ language prefix อัตโนมัติ

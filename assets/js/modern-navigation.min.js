 ...
 * - ENHANCED: Support language-prefixed URLs (/th/, /en/) with backward compatibility
 */
...
class ModernNavigation {
  constructor(config = {}) {
    ...
    // ✅ NEW: Supported language prefixes for URL detection
    this.supportedLangPrefixes = config.supportedLangPrefixes || ['th', 'en', 'ja', 'zh', 'ko'];
    this.langPrefixPattern = new RegExp(`^/(${this.supportedLangPrefixes.join('|')})/`);
  ...
  _onNavClick(event){
    ...
    // ✅ ENHANCED: Check active state with language prefix support
    if (this._isPathActive(targetLink)) {
      event.preventDefault();
      return;
    }
  ...
  /* --- ✅ NEW: URL Path Utilities for Multi-language Support --- */
  ...
  _normalizePath(path) {
    if (!path) return '/';
    // Remove language prefix (e.g., /th/, /en/)
    return path.replace(this.langPrefixPattern, '/');
  }

  /**
   * Check if current URL path matches the navigation link
   * Supports both old format (/page) and new format (/th/page, /en/page)
   * @param {string} navLink - The link from navigation config
   * @returns {boolean} Whether the link is active
   */
  _isPathActive(navLink) {
    if (!navLink) return false;
    
    const currentPath = window.location.pathname;
    
    // Normalize both paths (remove language prefixes)
    const normalizedCurrent = this._normalizePath(currentPath);
    const normalizedNav = this._normalizePath(navLink);
    
    // Handle root/index cases
    const isCurrentRoot = normalizedCurrent === '/' || normalizedCurrent.endsWith('/index.html');
    const isNavRoot = normalizedNav === '/' || normalizedNav === '/index.html';
    ...
    const navPathWithSlash = normalizedNav.endsWith('/') ? normalizedNav : `${normalizedNav}/`;
    const currentPathWithSlash = normalizedCurrent.endsWith('/') ? normalizedCurrent : `${normalizedCurrent}/`;
    
    return currentPathWithSlash.startsWith(navPathWithSlash) || normalizedCurrent === normalizedNav;
  }
  ...
  _updateActiveState(){
    ...
    for (let i = 0; i < items.length; i++){
      ...
      // ✅ ENHANCED: Use new path matching logic with language support
      const shouldBeActive = this._isPathActive(navLink);
...
(function(){
  const nav = new ModernNavigation({
    ...
    // ✅ NEW: Configure supported language prefixes (optional, defaults to ['th', 'en', 'ja', 'zh', 'ko'])
    supportedLangPrefixes: ['th', 'en', 'ja', 'zh', 'ko']
  });
  queueMicrotask(() => {

...
class ModernNavigation {
  constructor(config = {}) {
    ...
    // Ensure early same-window languageChange listener so we react immediately
    try {
      window.addEventListener('languageChange', (e) => {
        try {
          const lang = e?.detail?.language;
          if (lang) {
            this._onSelectedLangChange(lang);
          }
          // always resync URLs immediately when languageChange fired
          this._updateAllNavUrls();
          this._updateActiveState();
        } catch (err) { /* swallow */ }
      });

      // expose small public helper to force sync when needed
      this.syncLanguage = (lang) => {
        if (lang) {
          try { this.currentLang = lang; } catch (e) {}
        } else {
          try { this.currentLang = localStorage.getItem('selectedLang') || this.currentLang; } catch (e) {}
        }
  ...
  /**
   * สร้าง URL สำหรับ navigation link โดยอัตโนมัติเติม language prefixตาม selected/current language
  ...
  _createNavigationItemsFragment(){
    ...
    for (let i = 0; i < config.length; i++){
      ...
      // store canonical base URL so we can always regenerate prefixed URL later
      // (สำคัญ) เก็บ base URL ต้นฉบับไว้ใน dataset.baseUrl
  ...
  _updateAllNavUrls(){
    if (!this._navEl) return;
    // refresh currentLang from storage to be safe
    try {
      const storedLang = localStorage.getItem('selectedLang');
      if (storedLang && storedLang !== this.currentLang) this.currentLang = storedLang;
    } catch(e){}

    const items = this._navEl.querySelectorAll(this.navItemSelector);
    for (let i = 0; i < items.length; i++){
      const item = items[i];
      // choose canonical base url (prefer dataset.baseUrl if present)
      ...
      // generate new (possibly prefixed) path
      const newUrlPath = this._generateNavUrl(baseUrl);

      // normalize to absolute URL for reliable comparison and assignment
      let newAbsolute = newUrlPath;
      try {
        newAbsolute = new URL(newUrlPath, location.origin).toString();
      } catch (e) {
        // fallback: ensure leading slash
        newAbsolute = (newUrlPath && newUrlPath.startsWith('/')) ? (location.origin + newUrlPath) : newUrlPath;
      }

      // only assign if changed
      if (item.href !== newAbsolute) {
        try { item.href = newAbsolute; } catch (e) { /* swallow */ }
      }
    }
  }

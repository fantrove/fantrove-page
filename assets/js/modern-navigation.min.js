...
/* --- NavPrefixManager: centralized prefix & candidate logic --- */
class NavPrefixManager {
  ...
  parseUrlPath(path) {
    try {
      ...
      if (segments.length > 0 && this._isLangCode(segments[0])) {
        ...
        const clean = segments.slice(1).length ? '/' + segments.slice(1).join('/') : '/';
        return { lang, cleanPath: clean, originalPath: normalized, hasLangPrefix: true };
      }
      return { lang: null, cleanPath: normalized, originalPath: normalized, hasLangPrefix: false };
    } catch (e) {
      return { lang: null, cleanPath: path || '/', originalPath: path || '/', hasLangPrefix: false };
    }
  }

  // Generate URL for nav items by applying prefix rules (single responsibility)
  generateNavUrl(baseUrl) {
    try {
      if (!baseUrl) return '/';
      // Absolute URLs or protocol-relative or hash -> return as-is
      if (/^(http(s)?:)?\/\//.test(baseUrl) || baseUrl.startsWith('#')) return baseUrl;
      const normalizedBase = baseUrl.startsWith('/') ? baseUrl : '/' + baseUrl;
      if (this.isDevMode) return normalizedBase;

      const currentPath = window.location.pathname || '/';
      const parsed = this.parseUrlPath(currentPath);
      const preferredLang = parsed.lang || (this.readSelectedLang && this.readSelectedLang()) || this.defaultLang;

      // If baseUrl already has a supported lang prefix, return as-is
      const firstSeg = normalizedBase.split('/').filter(Boolean)[0];
      if (firstSeg && this._isLangCode(firstSeg)) return normalizedBase;

      // Prefix when preferredLang present
      if (preferredLang) return '/' + preferredLang + (normalizedBase === '/' ? '/' : normalizedBase);
  ...
  // Build candidate list for aggressive resolution (keeps ordering deterministic)
  buildCandidates(baseUrl) {
    const base = (baseUrl && baseUrl.startsWith('/')) ? baseUrl : '/' + (baseUrl || '');
    if (this.isDevMode) return [ base ];
    const parsedCurrent = this.parseUrlPath(window.location.pathname || '/');
    const langs = [];
    if (parsedCurrent.lang) langs.push(parsedCurrent.lang);
    const sel = this.readSelectedLang && this.readSelectedLang();
...
class ModernNavigation {
  constructor(config = {}) {
    ...
    // Instantiate NavPrefixManager (centralized URL/prefix logic)
    this._navPrefixManager = new NavPrefixManager({
      defaultLang: this.defaultLang,
      readSelectedLang: () => {
        try { return localStorage.getItem('selectedLang') || this.currentLang || this.defaultLang; } catch(e) { return this.currentLang || this.defaultLang; }
      },
   ...
   * Delegates to NavPrefixManager to centralize logic
   * @param {string} baseUrl - URL จาก config เช่น '/data/verse/discover'
   * @returns {string}
   */
  _generateNavUrl(baseUrl) {
    try {
      if (this._navPrefixManager) return this._navPrefixManager.generateNavUrl(baseUrl);
  ...
  _buildCandidatesForDestination(baseUrl) {
    // Delegate to NavPrefixManager
    if (this._navPrefixManager) return this._navPrefixManager.buildCandidates(baseUrl);
    // fallback: minimal candidate
  ...
  _createNavigationItemsFragment(){
    ...
    for (let i = 0; i < config.length; i++){
      ...
      // ✅ ใช้ baseUrl (raw) stored in data-base-url แล้วให้ NavPrefixManager สร้าง href
      const baseUrl = item.go_url || item.url || '#';
      // store raw base url for future re-generation
      link.dataset.baseUrl = baseUrl;
      // Backwards compatibility: keep old data fields if needed
      if (item.url) link.dataset.link = item.url;
      if (item.go_url) link.dataset.goUrl = item.go_url;
      
      // create href via navPrefixManager
      try {
        link.href = this._generateNavUrl(link.dataset.baseUrl);
      } catch (e) {
        link.href = (link.dataset.baseUrl && (link.dataset.baseUrl.startsWith('/') ? link.dataset.baseUrl : '/' + link.dataset.baseUrl)) || '/';
      }
  ...
  _onNavClick(event){
    ...
    const destination = goLink || targetLink || item.dataset.baseUrl;
  ...
  _updateAllNavUrls(){
    ...
    for (let i = 0; i < items.length; i++){
      const item = items[i];
      const baseUrl = item.dataset.baseUrl || item.dataset.goUrl || item.dataset.link;
      if (!baseUrl) continue;
      
      // ข้าม external links
      if (item.dataset.isExternal === 'true') continue;
      
      // สร้าง URL ใหม่ด้วย language prefix ปัจจุบัน (delegated)

 ...
 * ปรับปรุง: ลบการยึดตาม URL path prefix ทั้งหมด ยึดตาม localStorage.selectedLang อย่างเดียว
 * - ไม่สนใจ URL path อีกต่อไ�� (lang-proxy.js และ lang-links.js จัดการให้แล้ว)
 * - โฟกัสที่การ sync ภาษาจาก localStorage เป็นหลัก
 * - รองรับ marker @lsvg@, @svg@, @slot@, @a@, @br@, @strong@ เหมือนเดิม
 * - COORDINATION: ใช้ fv-forcereload / fv-reload-inflight / fv-reload-ack เพื่อหลีกเลี่ยง reload ซ้อนกัน
 */
...
class LanguageManager {
  constructor() {
    ...
    // Track language version for cross-tab sync
  ...
  _incLangVersion() {
    try {
      ...
      if (this._bc) {
        try {
          this._bc.postMessage({ lang: this.selectedLang, version: v });
        } catch(e){}
      }
      return v;
  ...
  _onBroadcastLang(msg) {
    try {
      ...
      if ((this.selectedLang || '') !== desiredLang) {
        this.updatePageLanguage(desiredLang).catch(() => {});
      }
    } catch (e) {}
  }

  // ----------------- NEW: session/history mapping & prediction helpers -----------------
  // Record language for current history entry (history.state) and into a session map
  recordLanguageForCurrentEntry(lang) {
    try {
      const key = (location.pathname || '/') + (location.search || '');
      // replace history state with lang tag (non-invasive)
      try {
        const st = history.state && typeof history.state === 'object' ? Object.assign({}, history.state) : {};
        st.lang = lang;
      ...
      try {
        ...
        map[key] = { lang, ts: Date.now(), evidence: 'language-change' };
        sessionStorage.setItem('fv-nav-lang-map', JSON.stringify(map));
      } catch (e) {}
    } catch (e) {}
  }

  // Record mapping for a specific path (used before navigation)
  recordLanguageForPath(path, lang) {
    try {
      if (!path) return;
      ...
      const raw = sessionStorage.getItem('fv-nav-lang-map') || '{}';
      const map = JSON.parse(raw || '{}');
      map[key] = { lang, ts: Date.now(), evidence: 'pre-nav' };
      sessionStorage.setItem('fv-nav-lang-map', JSON.stringify(map));
    } catch (e) {}
  }

  // Predict language for a given path using multiple heuristics (exact, normalized, parent, recent)
  getPredictedLangForPath(path) {
    try {
      ...
      // 3) parent path fallback: /a/b/c -> check /a/b, /a
      ...
      // 4) recent language-change heuristic: if user changed lang very recently on other page,
      // and the timestamp difference is small -> propagate
      try {
        ...
        if (Array.isArray(recent) && recent.length) {
          ...
          for (let i = recent.length - 1; i >= 0; i--) {
            const rec = recent[i];
            if (rec && rec.lang && (now - (rec.ts || 0) < 45000)) { // 45s window
              return rec.lang;
            }
            if (now - (rec.ts || 0) > 120000) break; // too old -> stop
          }
        }
      } catch (e) {}
  ...
  async fetchWithEtagOrHash(url, cacheKey, metaKey) {
    ...
    let resp = await fetch(url, { headers, cache: "reload", priority: "high" });
  ...
  async prepareAllButtonTexts() {
    ...
    // Restored richer globe-like SVG (improved vs previous simplified icon)
  ...
  // reload coordination helpers
  genReloadId() { return Date.now().toString(36) + '-' + Math.random().toString(36).slice(2, 9); }
  setForceReloadMarker(source) {
    try {
      const id = this.genReloadId();
      const marker = { id: id, ts: Date.now(), source: source || 'language' };
      sessionStorage.setItem('fv-forcereload', JSON.stringify(marker));
      return marker;
    } catch (e) { return null; }
  }
  setInflight(id) {
    try { if (!id) return; sessionStorage.setItem('fv-reload-inflight', id); } catch (e) {}
  }
  ackReload(id) {
    try {
      if (!id) return;
      sessionStorage.setItem('fv-reload-ack', id);
      // cleanup inflight & forcereload to avoid replays
      try { sessionStorage.removeItem('fv-reload-inflight'); } catch (e) {}
      try { sessionStorage.removeItem('fv-forcereload'); } catch (e) {}
    } catch (e) {}
  }

  // ลบ _pathHasLangPrefix, _prefixPathWithLang, _isLocalDev, _langFromPath ออกทั้งหมด
  // เพราะไม่ต้องสนใจ URL อีกต่อไป

  async handleInitialLanguage() {
    this.storeOriginalContent();
    // ยึดตาม localStorage เป็นหลัก ไม่สนใจ URL
    let storedLang = null;
    try { storedLang = localStorage.getItem('selectedLang'); } catch (e) { storedLang = null; }
    
    // ถ้าไม่มีใน localStorage ให้ detect จาก browser
    let autoLang = this.detectBrowserLanguage();
    this.selectedLang = (storedLang && this.languagesConfig[storedLang]) ? storedLang : (this.languagesConfig[autoLang] ? autoLang : 'en');
    
    // บันทึกกลับลง localStorage เพื่อให้ครั้งต่อไปใช้ค่านี้
  ...
  async initialize() {
    // Handle coordinated "force reload" marker set by proxy/navigation code to recover UI when URL prefix changes.
    try {
      ...
      if (markerRaw) {
        try {
          ...
          // If already acknowledged, just cleanup
          if (ack === marker.id) {
            try { sessionStorage.removeItem('fv-forcereload'); sessionStorage.removeItem('fv-reload-inflight'); } catch (e) {}
          } else if (inflight === marker.id) {
            // Someone already initiated navigation/reload -> acknowledge and do NOT reload again
            try { this.ackReload(marker.id); } catch (e) {}
            // proceed with normal initialization (no extra reload)
          } else {
            // No inflight -> we are responsible to perform a single replace reload
            try {
              // set inflight so other runners will not also reload
              this.setInflight(marker.id);
            } catch (e) {}
            try {
              // perform a replace to fetch a clean document from the host (no extra history entry)
              location.replace(location.href);
            } catch (e) {
              try { location.reload(); } catch (e2) {}
            }
            // return early; after reload the fresh page will ack
            return;
          }
        } catch (e) {
  ...
  async updatePageLanguage(language) {
    ...
    try {
      ...
      // บันทึกลง localStorage และเพิ่ม version
      ...
      // ไม่ต้อง update URL อีกต่อไป - ปล่อยให้ lang-proxy.js และ lang-links.js จัดการ
      // this.updateURLAndHistory(language, newVersion);
      
      // --- เพิ่มบันทึก mapping / recent change ---
      try {
        this.recordLanguageForCurrentEntry(language);
        // recent list for heuristic
        const raw = sessionStorage.getItem('fv-lang-recent') || '[]';
        const arr = JSON.parse(raw || '[]');
        arr.push({ path: location.pathname + (location.search||''), lang: language, ts: Date.now() });
        // cap size
        while (arr.length > 50) arr.shift();
        sessionStorage.setItem('fv-lang-recent', JSON.stringify(arr));
      } catch (e) {}
      // --------------------------------------------

      // NEW: Broadcast language change via CustomEvent so other in-page modules can react immediately
      try {
        window.dispatchEvent(new CustomEvent('languageChange', { detail: { language: this.selectedLang } }));
      } catch (e) {}

      // Trigger langSync to verify URL after language change (non-blocking)
      try {
        if (window.langSync && typeof window.langSync.verifyAndSyncUrl === 'function') {
          window.langSync.verifyAndSyncUrl({ force: true, reason: 'languageManager-update' }).catch(()=>{});
        }
      } catch (e) {}
  ...
  // ลบ updateURLAndHistory หรือทำให้เป็น no-op
  updateURLAndHistory(language, version) {
    // ไม่ทำอะไร - ปล่อยให้ระบบอื่นจัดการ URL
    // เก็บไว้เพื่อ compatibility กับโค้ดเก่าที่อาจเรียกใช้
  }

  setupNavigationHandlers() {
    // ลดความซับซ้อนของ popstate handler - ไม่ต้องสนใจ URL prefix อีกต่อไป
    const popHandler = async (event) => {
      // First, try to use event.state.lang or session prediction before fallback checks
      try {
        // prefer event.state.lang when available
        const st = event && event.state && typeof event.state === 'object' ? event.state : null;
        if (st && st.lang && st.lang !== this.selectedLang) {
          await this.updatePageLanguage(st.lang);
          // After updating due to URL/history language change, coordinate reload:
          try {
            const marker = this.setForceReloadMarker('popstate');
            if (marker && marker.id) {
              // mark inflight and then reload - coordinated so landing page will ack
              this.setInflight(marker.id);
            }
          } catch (e) {}
        ...
        // prediction based on session map
        try {
          const predicted = this.getPredictedLangForPath(location.pathname + (location.search||''));
          if (predicted && predicted !== this.selectedLang) {
            await this.updatePageLanguage(predicted);
            try {
              const marker = this.setForceReloadMarker('prediction');
              if (marker && marker.id) this.setInflight(marker.id);
            } catch (e) {}
            try { location.reload(); } catch(e){}
            return;
          }
      ...
      try {
        // ตรวจสอบเฉพาะ localStorage และ langVersion
        ...
        if (storedVersion !== this._langVersion || storedLang !== this.selectedLang) {
          ...
          await this.updatePageLanguage(storedLang);
        }
      } finally {
        this.isNavigating = false;
      }
      ...
      try {
        if (e.key === 'selectedLang' || e.key === 'langVersion') {
          ...
          if (newVersion !== this._langVersion || newLang !== this.selectedLang) {
            ...
            this.updatePageLanguage(newLang).catch(() => {});
          }
        }
      } catch (e) {}
    };
    ...
    // visibility/focus - re-check เมื่อกลับมาที่ tab
    const visibilityHandler = () => {
      try {
        if (document.visibilityState === 'visible') {
          ...
          if (storedVersion !== this._langVersion || storedLang !== this.selectedLang) {
            this._langVersion = storedVersion;
            this.updatePageLanguage(storedLang).catch(() => {});
          }
        }
      } catch (e) {}
    };
    ...
    // NOTE: We intentionally DO NOT override localStorage.setItem here.
    // Use BroadcastChannel + storage event + languageChange custom event to notify other modules.
  ...
  _onSelectedLangChange(lang) {
    if (lang && lang !== this.selectedLang) {
      this.updatePageLanguage(lang).catch(() => {});
    }
  }
  ...
  async resetToEnglishContent() {
    ...
    const resetPromises = Array.from(elements).map(el => new Promise(resolve => {
      requestAnimationFrame(() => {
            ...
            function splitMarkersAndHtml(str) {
              ...
              for (let segment of htmlSplit) {
                ...
                if (/^<\\/?.+?>$/.test(segment)) {
                  ...
                  let m;
                  while ((m = markerRegex.exec(segment)) !== null) {
                    if (m.index > lastIndex) parts.push({ type: 'text', text: segment.slice(lastIndex, m.index)});
                    if (m[1]) {
                      const id = m[2] || null;
                      parts.push({ type: 'lsvg', id });
                    } else if (m[3]) {
                  ...
                  if (lastIndex < segment.length) parts.push({ type: 'text', text: segment.slice(lastIndex) });
                }
              }
              return parts;
            }
        ...
        if (originalStyle) el.style.cssText = originalStyle;
      });
    }));

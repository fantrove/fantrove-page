...
`;
  ...
  async handleInitialLanguage() {
    ...
    // NEW: if enforcePathLangPrefix is enabled, and current path lacks prefix, attempt to replace URL to prefixed form
    try {
      if (this.enforcePathLangPrefix) {
        const curPath = window.location.pathname || '/';
        if (!this._pathHasLangPrefix(curPath)) {
          // If running on local dev, skip promotion to avoid dev-server read errors
          if (this._isLocalDev()) {

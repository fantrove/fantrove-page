/**
 * language.min.js (modified full source)
 * LanguageManager: advanced language loading, translation, and coordination
 * - Uses IndexedDB cache for language JSON
 * - Worker pool for parsing marker-rich translation strings
 * - BroadcastChannel + storage-based coordination across tabs
 * - Emits 'languageChange' CustomEvent and integrates with lang-sync verifier
 *
 * Note: This is a formatted (non-minified) readable source for development.
 */
(function (global) {
  'use strict';
  ...
  class LanguageManager {
    constructor() {
      ...
      // Worker code: parse markers and return parts
      const workerCode = `
            ...
            if (/^<\\/?.+?>$/.test(segment)) {
              ...
              while ((m = markerRegex.exec(segment)) !== null) {
                ...
                if (m[1]) {
                  ...
                  parts.push({ type: 'a', translate, text: inner });
                } else if (m[9]) {
                  parts.push({ type: 'br' });
                } else if (m[10]) {
                  const s = m[11] || '';
    ...
    // ----------------- session/history mapping & prediction helpers -----------------
    ...
    getPredictedLangForPath(path) {
      try {
        ...
        // 4) recent language-change heuristic
        try {
    ...
    async fetchWithEtagOrHash(url, cacheKey, metaKey) {
      ...
      let resp = await fetch(url, { headers, cache: "reload", credentials: "same-origin" });
    ...
    prepareAllButtonTexts() {
      ...
      // Small simple globe SVG for button
      ...
      Object.entries(this.languagesConfig).forEach(([lang, config]) => {
        ...
        span.textContent = config.buttonText || config.label || lang;
    ...
    async loadLanguagesConfig() {
      ...
      if (!this.languagesConfig || !Object.keys(this.languagesConfig).length)
        throw new Error("Config invalid or missing");
    ...
    async initialize() {
        ...
        if (markerRaw) {
          try {
            ...
            if (ack === marker.id) {
              ...
              try { this.setInflight(marker.id); } catch (e) {}
              try { location.replace(location.href); } catch (e) { try { location.reload(); } catch (e2) {} }
              return;
            }
          } catch (e) {}
        }
      ...
      try {
        ...
        this.showError('Unable to initialize language system');
        setTimeout(() => { if (document.body && document.body.style.opacity === "0") document.body.style.opacity = "1"; }, 0);
      }
    }
    ...
    async updatePageLanguage(language) {
      ...
      try {
        ...
        // Trigger langSync if available: user-initiated changes should allow redirect
        try {
          if (window.langSync && typeof window.langSync.verifyAndSyncUrl === 'function') {
            window.langSync.verifyAndSyncUrl({ force: true, reason: 'languageManager-update', allowRedirect: true, userInitiated: true }).catch(()=>{});
          }
        } catch (e) {}
    ...
    updateURLAndHistory(language, version) {
      // no-op for compatibility (URL managed by lang-proxy/lang-sync)
    }
        ...
        requestAnimationFrame(() => {
          ...
          if (originalText) {
            const workerCode = `
              ...
              self.onmessage = function(e) { self.postMessage(splitMarkersAndHtml(e.data)); }
            `;

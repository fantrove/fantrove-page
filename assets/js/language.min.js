...
`;
  ...
  async updatePageLanguage(language, options = { updateHistory: true }) {
    ...
    try {
      ...
      this.showError('เกิดข้อผิดพลาดในการเ���ลี่ยนภาษา');
      await this.resetToEnglishContent();
    } finally {
      this.isUpdatingLanguage = false;
    }
  ...
  setupNavigationHandlers() {
    ...
    // - If desiredLang (from URL prefix or selectedLang in storage) != current selectedLang
    //   then on non-local hosts we force a full reload so that server resources + URL + UI stay in-sync.
    window.addEventListener('popstate', async (event) => {
      ...
      try {
        ...
        const desiredLang = langInUrl || storedLang || 'en';
        if (desiredLang !== this.selectedLang) {
          // On production (non-local) enforce a full reload so server-side resources and URL stay in sync.
          if (!this._isLocalDev()) {
            try { window.location.reload(); return; } catch(e){}
          } else {
            // dev: retain previous dynamic behavior (do not reload)
            await this.updatePageLanguage(desiredLang, { updateHistory: false });
          }
        }
      } finally {
        this.isNavigating = false;

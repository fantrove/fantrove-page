/**
 * LanguageManager Ultra-Enterprise: Smart URL Sync Edition
 * ปรับปรุง: ทำงานร่วมกับ lang-proxy.js, lang-sync.js, lang-links.js
 * - ไม่จัดการ URL prefix เอง ปล่อยให้ระบบภายนอกจัดการ
 * - แจ้งเตือนเมื่อมีการเปลี่ยนภาษาเพื่อให้ระบบอื่นซิงค์
 * - รองรับการซิงค์ระหว่าง tabs และการนำทาง back/forward
 */
...
class LanguageManager {
  constructor() {
    ...
    try { 
      this._bc = (typeof BroadcastChannel !== 'undefined') ? new BroadcastChannel('fv-lang') : null; 
    } catch (e) { 
      this._bc = null; 
    }
  ...
  _readLangVersion() {
    try { 
      return Number(localStorage.getItem('langVersion') || 0); 
    } catch (e) { 
      return 0; 
    }
  }
  ...
  _onBroadcastLang(msg) {
    try {
      ...
      const desiredLang = (lang && this.languagesConfig && this.languagesConfig[lang]) 
        ? lang 
  ...
  // Session/history mapping helpers
      ...
      try {
        ...
        if (Array.isArray(recent) && recent.length) {
          ...
          for (let i = recent.length - 1; i >= 0; i--) {
            const rec = recent[i];
            if (rec && rec.lang && (now - (rec.ts || 0) < 45000)) {
              return rec.lang;
            }
  ...
  async handleInitialLanguage() {
    ...
    this.selectedLang = (storedLang && this.languagesConfig[storedLang]) 
      ? storedLang 
  ...
  async initialize() {
    ...
    try {
      ...
      // แจ้งระบบภายนอกว่า LanguageManager พร้อมแล้ว
      window.dispatchEvent(new CustomEvent('languageManagerReady', { 
        detail: { language: this.selectedLang } 
      }));
  ...
  async updatePageLanguage(language) {
    ...
    try {
      ...
      window.dispatchEvent(new CustomEvent('languageChange', { 
        detail: { language: this.selectedLang, source: 'languageManager' } 
      }));
  ...
  setupNavigationHandlers() {
    // ตรวจสอบภาษาเมื่อ tab กลับมา active
    ...
    // Storage sync (จาก tab อื่น)
    window.addEventListener('storage', (e) => {
      if (e.key === 'selectedLang' && e.newValue !== this.selectedLang) {
        this.updatePageLanguage(e.newValue).catch(() => {});
      }
    });
    
    // ฟังก์ชันสำหรับระบบภายนอกเรียกใช้
    window.addEventListener('languageChange', (e) => {
      if (e.detail && e.detail.language && e.detail.source !== 'languageManager') {
        if (e.detail.language !== this.selectedLang) {
          this.updatePageLanguage(e.detail.language).catch(() => {});
        }
      }
    });
  }
      ...
      if (p.type === 'text') {
        ...
        if (/\<svg[\s>]/i.test(htmlStr)) {
          try {
            ...
            const svgRoot = svgDoc.documentElement && svgDoc.documentElement.nodeName !== 'parsererror' 
  ...
  async selectLanguage(language) {
    ...
    // แจ้งระบบอื่นทันที
    window.dispatchEvent(new CustomEvent('languageChange', { 
      detail: { language: language, source: 'user-select' } 
    }));
    
    // เรียกใช้ LangSync ถ้ามี
    if (window.LangSync && window.LangSync.onLanguageChange) {
      window.LangSync.onLanguageChange(language);
    }
    
    // เรียกใช้ LangLinks ถ้ามี
    if (window.LangLinks && window.LangLinks.updateLanguage) {
      window.LangLinks.updateLanguage(language);
    }
    
    await this.updatePageLanguage(language);
    await this.closeLanguageDropdown();
    
    // ตรวจสอบ URL อีกครั้งหลังจากอัพเดท
    setTimeout(() => {
      const currentPath = location.pathname;
      const expectedPrefix = `/${language}/`;
      const hasCorrectPrefix = currentPath.startsWith(expectedPrefix) || currentPath === `/${language}`;
      
      if (!hasCorrectPrefix) {
        // URL ไม่ตรง ให้ LangSync จัดการ
        if (window.LangSync && window.LangSync.sync) {
          window.LangSync.sync();
        }
      }
    }, 100);
  }

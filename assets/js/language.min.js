...
class LanguageManager {
  ...
  async initialize() {
    // Handle one-time "force reload" marker set by proxy/navigation code to recover UI when URL prefix changes.
    try {
      const fr = sessionStorage.getItem('fv-forcereload');
      const frLast = sessionStorage.getItem('fv-forcereload-last');
      if (fr && fr !== frLast) {
        // Mark that we've observed this marker and perform a single reload to get a clean fetch from host.
        try { sessionStorage.setItem('fv-forcereload-last', fr); } catch (e) {}
        try {
          // Replace (no history entry) to fetch a clean document from the server/host.
          location.replace(location.href);
        } catch (e) {
          try { location.reload(); } catch (e2) {}
        }
        return;
      } else if (fr && fr === frLast) {
        // cleanup markers after the one reload has happened
        try { sessionStorage.removeItem('fv-forcereload'); sessionStorage.removeItem('fv-forcereload-last'); } catch (e) {}
      }
    } catch (e) {}
    ...
    const popHandler = async (event) => {
      ...
      try {
        ...
        if (st && st.lang && st.lang !== this.selectedLang) {
          ...
          // After updating due to URL/history language change, force a reload so UI is loaded cleanly from host.
        ...
        try {
          ...
          if (predicted && predicted !== this.selectedLang) {
            ...
            // After predicted language switch triggered by navigation, force reload for UI consistency
            try { sessionStorage.setItem('fv-forcereload', String(Date.now())); } catch(e){}
            try { location.reload(); } catch(e){}
            return;
          }

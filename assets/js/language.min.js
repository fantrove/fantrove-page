 ...
 * - หากการแปลไม่ระบุการจัดวาง SVG ใด ๆ (ไม่มี @svg@/@lsvg@ และไม่มี <svg> ในเนื้อหาแปล) ระบบจะพยายาม "predicted lsvg" เพื่อ preserve svg ตัวแรก และวางไว้ข้างหน้า
 * - หากมีการระบุ @lsvg@ ในสตริงแปล จะยึดตำแหน่งนั้น (แต่ยังคงความพยายามในการ preserve identity ของ <svg> เพื่อรักษา event listeners / state)
 * - ระบบ reconciliation ยังคงพยายาม preserve identity ของ <svg> (ไม่ replace element ถ้า match) เพื่อรักษา event listeners / state
 *
 * เพิ่มเติม (ปรับปรุงสำหรับการบังคับ prefix ภาษาใน path)
 * - this.enforcePathLangPrefix: เมื่อเป็น true การเรียก updatePageLanguage จะพยายามปรับ URL ให้มี language prefix ใน path (เ�[...]
 * - handleInitialLanguage จะพยายาม promote (replace) ผู้ใช้ไปยัง path ที่มี prefix หาก selectedLang มีค่า
 *
 * พฤติกรรมโดยย่อ:
 * - ถ้ามีข้อมูลการวาง svg ตัวใดตัวหนึ่งในสตริงแปล (เช่น @svg@, @svg:id@, @lsvg@ หรือ inline <svg>), ระบบจะใช้ตำแหน่งที่ระบุ
 * - ถ้าไม่มีข้อมูลใด ๆ แต่ DOM มี <svg> อยู่แล้ว ระบบจะ "คาดคะเน" ว่า svg ตัวแรก (หรือที่ match) ควรถูก preserve/moved ให้เหมาะสม
 *
...
class LanguageManager {
  constructor() {
    ...
    // NEW: whether we should enforce path-based language prefixes (true by default)
    this.enforcePathLangPrefix = true;

    // New: track language version to detect real changes and coordinate across tabs
    ...
    if (this._bc) this._bc.onmessage = (ev) => this._onBroadcastLang(ev.data);
  ...
  // language version helpers (stored in localStorage as 'langVersion')
  _readLangVersion() {
    try { return Number(localStorage.getItem('langVersion') || 0); } catch (e) { return 0; }
  }
  _incLangVersion() {
    try {
      const v = this._readLangVersion() + 1;
      localStorage.setItem('langVersion', String(v));
      this._langVersion = v;
      if (this._bc) {
        try {
          // include canonical URL (with current pathname/search/hash) so receivers can replaceState safely
          const canonicalUrl = window.location.origin + window.location.pathname + window.location.search + window.location.hash;
          this._bc.postMessage({ lang: this.selectedLang, version: v, url: canonicalUrl });
        } catch(e){}
      }
      return v;
    } catch (e) { return this._langVersion; }
  }
  _onBroadcastLang(msg) {
    try {
      if (!msg || typeof msg !== 'object') return;
      const { lang, version, url } = msg;
      if (typeof version !== 'number' || version === this._langVersion) return;

      // adopt version immediately to avoid race
      this._langVersion = version;

      // pick language from message, fallback to localStorage
      const desiredLang = (lang && this.languagesConfig && this.languagesConfig[lang]) ? lang : (localStorage.getItem('selectedLang') || 'en');

      // If url is provided and different, replace history state to that url (do not reload)
      try {
        if (url && typeof url === 'string') {
          const state = { language: desiredLang, lastLanguage: this.lastSelectedLang, timestamp: Date.now(), langVersion: version };
          try { history.replaceState(state, '', url); } catch(e){}
        }
      } catch (e) {}

      // If language differs, attempt dynamic update (non-blocking)
      if ((this.selectedLang || '') !== desiredLang) {
        // updatePageLanguage will update content and call updateURLAndHistory (updateHistory:false here since we've replaced state)
        this.updatePageLanguage(desiredLang, { updateHistory: false }).catch(err => {
          // If dynamic update fails and we are not local dev, do a reload as last resort
          try {
            if (!this._isLocalDev()) { try { window.location.reload(); } catch(e){} }
          } catch(e){}
        });
      }
    } catch (e) {}
...
`;
  ...
  /* New helper: detect if a path has a language prefix using configured languages */
  ...
  /* New helper: prefix or replace prefix on a path with a language */
  ...
  /* New helper: detect local/dev hosts to avoid promotion during development */
  _isLocalDev() {
    try {
      ...
      // consider DEV_PORT (optional) environment: if running on common dev ports treat as dev
      if (location.port && ['3000','5173','7700','8080','3001'].includes(String(location.port))) return true;
      return false;
    } catch (e) { return false; }
  }

  /* New helper: extract language code from path if present */
  _langFromPath(path) {
    try {
  ...
  async handleInitialLanguage() {
    ...
    // Priority: query param -> path prefix -> storedLang -> browser detection -> 'en'
    this.selectedLang =
      (langFromQuery && this.languagesConfig[langFromQuery]) ? langFromQuery :
      (langFromPath && this.languagesConfig[langFromPath]) ? langFromPath :
    ...
    // If enforcePathLangPrefix is enabled, try to ensure path and selectedLang are in sync.
    try {
      if (this.enforcePathLangPrefix) {
        const curPath = window.location.pathname || '/';

        // If path has a lang prefix but it differs from selectedLang, prefer the path and persist it
        if (langFromPath && langFromPath !== this.selectedLang) {
          this.selectedLang = langFromPath;
          try { localStorage.setItem('selectedLang', this.selectedLang); } catch(e){}
          this.showButtonTextForLang(this.selectedLang);
        }

        // If path lacks prefix, attempt to promote (same logic as before) unless dev
        if (!this._pathHasLangPrefix(curPath)) {
          if (this._isLocalDev()) {
            // do not redirect/promote in dev
          } else {
            // prevent repeated redirects within same session
            const key = 'lm-promote-prefix:' + this.selectedLang + ':' + curPath;
            if (!sessionStorage.getItem(key)) {
              sessionStorage.setItem(key, '1');
              // compute target path
              const targetPath = this._prefixPathWithLang(curPath, this.selectedLang);
              // preserve search & hash
              const urlObj = new URL(window.location.origin + targetPath + window.location.search + window.location.hash);
              try {
                // use replace to avoid creating history entries
                window.location.replace(urlObj.toString());
                return; // stop further initialization on this page (we will re-run on the new URL)
              } catch (e) {}
            }
          }
        }
  ...
  async loadLanguagesConfig() {
    ...
    if (!this.languagesConfig || !Object.keys(this.languagesConfig).length)
      throw new Error("Config ไม่ถูกต���องและโหลดใหม่ไม่ได้");
  ...
  async updatePageLanguage(language, options = { updateHistory: true }) {
    ...
    try {
      ...
      // Bump version and persist selectedLang
      try { localStorage.setItem('selectedLang', language); } catch(e){}
      const newVersion = this._incLangVersion();

      this.selectedLang = language;
      this.showButtonTextForLang(this.selectedLang);
      if (options.updateHistory) this.updateURLAndHistory(language, newVersion);
    } catch (error) {
  ...
  updateURLAndHistory(language, version) {
    try {
      if (this.enforcePathLangPrefix) {
        ...
        if (language === 'en') {
          ...
          if (this._pathHasLangPrefix(curPath)) {
            ...
            parts.splice(1,1); // remove the lang segment
            newPath = parts.join('/') || '/';
          }
        } else {
          // ensure path is prefixed
          if (!this._pathHasLangPrefix(curPath)) {
            newPath = this._prefixPathWithLang(curPath, language);
          } else {
            // replace existing prefix with new language
            newPath = this._prefixPathWithLang(curPath, language);
          }
        }
        // preserve search & hash
        const newUrlObj = new URL(window.location.origin + newPath + url.search + url.hash);
        const state = { language, lastLanguage: this.lastSelectedLang, timestamp: Date.now(), langVersion: version || this._langVersion };
        try {
          // Always replaceState so history entry and state are consistent across tabs/popstate handling
          history.replaceState(state, '', newUrlObj.toString());
        } catch (e) {
          // fallback
          try { history.replaceState(state, '', newPath + url.search + url.hash); } catch (e2) {}
        }
        return;
      }
    } catch (e) {
      // fallback to query param behavior below
    }

    // fallback / legacy: query param approach
    const url = new URL(window.location.href);
  ...
  setupNavigationHandlers() {
    // On popstate we now strictly enforce language/URL matching on production:
    // - If event.state.langVersion or URL-prefix/selectedLang differs from current -> attempt dynamic sync, reload only as fallback
    const popHandler = async (event) => {
      if (this.isLanguageDropdownOpen) await this.closeLanguageDropdown();
      if (this.isNavigating) return;
      this.isNavigating = true;
      try {
        // 1) check event.state if present (preferred, carries langVersion)
        const state = event.state || {};
        const stateVersion = state && state.langVersion ? Number(state.langVersion) : null;
        const stateLang = state && state.language ? state.language : null;

        // 2) determine language implied by URL path
        const langInUrl = this._langFromPath(window.location.pathname);

        // 3) current known language/version
        const storedLang = (function(){try{return localStorage.getItem('selectedLang');}catch(e){return null;}})() || 'en';
        const storedVersion = this._readLangVersion();

        // Decide if real language change happened:
        // - if stateVersion exists and differs -> sync
        // - else if langInUrl exists and langInUrl !== this.selectedLang -> sync
        // - else if storedLang !== this.selectedLang && storedVersion !== this._langVersion -> sync
        let needSync = false;
        if (stateVersion !== null && stateVersion !== this._langVersion) needSync = true;
        else if (langInUrl && langInUrl !== this.selectedLang) needSync = true;
        else if (storedLang && storedLang !== this.selectedLang && storedVersion !== this._langVersion) needSync = true;

        if (needSync) {
          const desired = stateLang || langInUrl || storedLang || 'en';
          // Attempt a dynamic update first
          try {
            this._langVersion = stateVersion !== null ? stateVersion : this._readLangVersion();
            await this.updatePageLanguage(desired, { updateHistory: false });
            // Ensure history state reflects current language/version/url
            const canonicalUrl = window.location.origin + window.location.pathname + window.location.search + window.location.hash;
            const st = { language: this.selectedLang, lastLanguage: this.lastSelectedLang, timestamp: Date.now(), langVersion: this._langVersion };
            try { history.replaceState(st, '', canonicalUrl); } catch(e){}
          } catch (err) {
            // fallback: reload on non-dev hosts
            if (!this._isLocalDev()) {
              try { window.location.reload(); return; } catch(e){}
            } else {
              // dev: perform dynamic update but swallow errors
              try { await this.updatePageLanguage(desired, { updateHistory: false }); } catch(e){}
            }
          }
        } else {
          // no language change — still allow potential dynamic update when history contains language
          if (stateLang && stateLang !== this.selectedLang) {
            try {
              await this.updatePageLanguage(stateLang, { updateHistory: false });
            } catch (e) {
              if (!this._isLocalDev()) { try { window.location.reload(); return; } catch(e){} }
            }
          }
        }
      } finally {
    ...
    const storageHandler = (e) => {
      try {
        if (e.key === 'selectedLang' || e.key === 'langVersion') {
          ...
          const newVersion = this._readLangVersion();
          if (newVersion !== this._langVersion || newLang !== this.selectedLang) {
            this._langVersion = newVersion;
            // Attempt dynamic sync
            this.updatePageLanguage(newLang, { updateHistory: false }).catch(() => {
              if (!this._isLocalDev()) { try { window.location.reload(); } catch(e){} }
            });
          }
        }
    ...
    // visibility/focus: when user returns to tab, re-check selectedLang/langVersion
    const visibilityHandler = () => {
      try {
        if (document.visibilityState === 'visible') {
          const storedVersion = this._readLangVersion();
          const storedLang = localStorage.getItem('selectedLang') || 'en';
          if (storedVersion !== this._langVersion || storedLang !== this.selectedLang) {
            this._langVersion = storedVersion;
            if (!this._isLocalDev()) {
              try { this.updatePageLanguage(storedLang, { updateHistory: false }).catch(()=>{}); } catch(e){}
            } else {
              this.updatePageLanguage(storedLang, { updateHistory: false }).catch(()=>{});
            }
          }
        }
      } catch (e) {}
    ...
    window.addEventListener('languageChange', (e) => {
      if (e?.detail?.language) {
        ...
        // Update URLs when language changes
        this.updateURLAndHistory(e.detail.language, this._langVersion);
        this._updateAllNavUrls && this._updateAllNavUrls();
      }
    });
  }

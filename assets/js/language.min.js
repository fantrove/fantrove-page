 ...
 * - หากการแปลไม่ระบุการจัดวาง SVG ใด ๆ (ไม่มี @svg@/@lsvg@ และไม่มี <svg> ในเนื้อหาแปล) ร��[...]
...
class LanguageManager {
  ...
  _incLangVersion() {
    try {
      ...
      this._langVersion = v;
      if (this._bc) try { this._bc.postMessage({ lang: this.selectedLang, version: v }); } catch(e){}
      return v;
  ...
  _onBroadcastLang(msg) {
    try {
      ...
      if (typeof version === 'number' && version !== this._langVersion) {
        if (!this._isLocalDev()) { try { window.location.reload(); } catch(e){}; }
        else { this.updatePageLanguage(lang || localStorage.getItem('selectedLang') || 'en', { updateHistory: false }).catch(()=>{}); }
      }
    } catch (e) {}
  }
...
async handleInitialLanguage() {
  ...
  // ensure UI reflects chosen language
  ...
  // If path already has a prefix that differs from stored, prefer the path (and persist it).
  try {
    if (this.enforcePathLangPrefix) {
      const curPath = window.location.pathname || '/';
      
      // If path has a lang prefix but it differs from what we had (e.g., storedLang),
      // prefer the path language and persist it so all systems agree.
      ...
      if (!this._pathHasLangPrefix(curPath)) {
        if (this._isLocalDev()) {
          ...
          if (!sessionStorage.getItem(key)) {
            ...
            try {
              // replace so we don't create a history entry and re-run initialization on the new URL
              window.location.replace(urlObj.toString());
              return; // stop initialization on this page; we'll re-run on the prefixed URL
            } catch (e) {}
          }
        }
      }
  ...
  // Finally, if not English (or if en source is json) apply translations.
  ...
  setupNavigationHandlers() {
    ...
    // - If event.state.langVersion or URL-prefix/selectedLang differs from current -> reload (production)
    window.addEventListener('popstate', async (event) => {
      ...
      try {
        ...
        // - if stateVersion exists and differs -> reload (production)
        // - else if langInUrl exists and langInUrl !== this.selectedLang -> reload
        // - else if storedLang !== this.selectedLang && storedVersion !== this._langVersion -> reload
        let needReload = false;
        if (stateVersion !== null && stateVersion !== this._langVersion) needReload = true;
        else if (langInUrl && langInUrl !== this.selectedLang) needReload = true;
        else if (storedLang && storedLang !== this.selectedLang && storedVersion !== this._langVersion) needReload = true;

        if (needReload) {
          if (!this._isLocalDev()) {
            try { window.location.reload(); return; } catch (e) {}
          } else {
            // dev: perform dynamic update instead of reload
          ...
          if (stateLang && stateLang !== this.selectedLang) {
            if (this._isLocalDev()) await this.updatePageLanguage(stateLang, { updateHistory: false });
            else { try { window.location.reload(); return; } catch(e){} }
          }
        }
      } finally {
        this.isNavigating = false;
    ...
    window.addEventListener('storage', (e) => {
      try {
        if (e.key === 'selectedLang' || e.key === 'langVersion') {
          ...
          if (newVersion !== this._langVersion) {
            if (!this._isLocalDev()) {
              try { window.location.reload(); } catch(e){}
            } else {
              this.updatePageLanguage(newLang || 'en', { updateHistory: false }).catch(()=>{});
            }
          }
        }
      } catch (e) {}
      ...
      try {
        if (document.visibilityState === 'visible') {
          ...
          if (storedVersion !== this._langVersion || storedLang !== this.selectedLang) {
            if (!this._isLocalDev()) { try { window.location.reload(); } catch(e){} }
            else this.updatePageLanguage(storedLang, { updateHistory: false }).catch(()=>{});
          }
        }
      } catch (e) {}
    });

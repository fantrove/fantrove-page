class FantroveConsolePro {
    constructor() {
        ...
        this.cloudLoadAttempted = false; // ตัวแปรใหม่เพื่อป้องกันการเรียกซ้ำ
    ...
    init() {
        ...
        // โหลดข้อมูลจากทุกแหล่งพร้อมกัน แต่ใช้ try-catch แยกกัน
        this.loadAllSourcesSafe();
        
        // ใช้ skipStorage = true เพื่อไม่บันทึกลงฐานข้อมูล
        this.system('Console ready (Session: ' + this.sessionId.substring(0, 8) + ')', null, true);
        
        // เริ่มระบบ Real-time Sync (แบบเบา)
        this.startRealTimeSyncSafe();
    }

    // โหลดข้อมูลแบบปลอดภัย ไม่ให้ error ที่จุดใดจุดหนึ่งทำให้ทั้งระบบล้ม
    async loadAllSourcesSafe() {
        this.setSyncStatus(true, 'Loading...');
        
        // 1. โหลดจาก Local ก่อน (ไม่มีปัญหา)
        await this.loadFromLocalBackup();
        
        // 2. โหลดจาก Cloud แบบปลอดภัย
        if (this.isOnline && !this.cloudLoadAttempted) {
            try {
                await this.loadLogsFromCloudSafe();
            } catch (err) {
                console.warn('Cloud load failed (safe mode):', err.message);
                // ไม่ throw error ต่อ ให้ทำงานต่อด้วย Local data
            }
        }
    ...
    mergeAndSortLogs() {
        ...
        this.logs = uniqueLogs.slice(0, 500); // ลดจาก 1000 เหลือ 500 เพื่อความเสถียร
    }
    ...
    async connectToCloud() {
        ...
        try {
            ...
            this.isCloudConnected = false;
            this.updateConnectionStatus('local');
            // ไม่ throw error ให้ระบบทำงานต่อได้
        }
    }

    async retryConnection() {
        this.hideError();
        this.cloudLoadAttempted = false; // รีเซ็ต flag เพื่อให้ลองใหม่ได้
        
        try {
            await this.connectToCloud();
            this.showToast('Reconnected successfully');
        } catch (error) {
            this.showError('Cannot connect to cloud, working in local mode');
        }
    }

    // ดึงข้อมูลจาก Cloud แบบปลอดภัย (แก้ไขปัญหาหลัก)
    async loadLogsFromCloudSafe() {
        if (!this.isOnline || this.cloudLoadAttempted) {
            return [];
        }

        this.cloudLoadAttempted = true; // ตั้ง flag ว่าได้ลองแล้ว
        
        // ตรวจสอบ Health ก่อนแบบง่ายๆ ไม่ต้องซับซ้อน
        try {
        ...
        // ดึงข้อมูลแบบค่อยเป็นค่อยไป ใช้ limit น้อยๆ ก่อน (100 แทน 1000)
        const limit = 100; // ลดจาก 1000 เหลือ 100 เพื่อหลีกเลี่ยง Error 500
        
        try {
            const response = await fetch(
                `${this.apiUrl}/logs?session=${encodeURIComponent(this.sessionId)}&limit=${limit}`,
                {
                    method: 'GET',
                    headers: { 
            ...
            if (!response.ok) {
                // ถ้าเป็น 500 ให้ลองใหม่ด้วย limit น้อยลง
                if (response.status === 500) {
                    console.warn('Server error 500, retrying with smaller limit...');
                    return await this.loadLogsFromCloudRetry();
                }
            ...
            if (!Array.isArray(logs)) {
                console.warn('Invalid response format from cloud');
                return [];
            }
            ...
            const cloudLogs = logs.map(log => ({
                ...
                fromCloud: true
            })).filter(log => log.timestamp && log.message); // กรองข้อมูลที่ไม่สมบูรณ์ออก
    ...
    // ลองใหม่ด้วย limit น้อยลงเมื่อเจอ Error 500
    async loadLogsFromCloudRetry() {
        const limit = 50; // ลดเหลือ 50
        ...
        try {
            ...
            if (cloudLogs.length > 0) {
                this.system(`Loaded ${cloudLogs.length} logs from cloud (retry)`, null, true);
            }
    ...
    async syncPendingLogs() {
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (saved > 0) {
                    this.system(`Synced ${saved}/${batch.length} logs${failed > 0 ? ` (${failed} failed)` : ''}`, null, true);
                }
    ...
    async syncAll() {
        ...
        // 2. รีเซ็ต flag และโหลดจาก Cloud ใหม่
        this.cloudLoadAttempted = false;
        try {
            await this.loadLogsFromCloudSafe();
        } catch (e) {
            console.warn('Full sync cloud load failed:', e);
        }
        ...
        this.setSyncStatus(false);
        this.showToast(`Total logs: ${this.logs.length}`);
    }

    // Real-time Sync แบบเบา (ไม่ดึงข้อมูลหนัก)
    startRealTimeSyncSafe() {
        // Sync ทุก 30 วินาที
        this.syncInterval = setInterval(() => {
            if (!document.hidden && this.isOnline) {
                this.syncPendingLogs();
                // ไม่ดึงข้อมูลใหม่จาก Cloud อัตโนมัติบ่อยๆ เพื่อหลีกเลี่ยง Error 500
                // ให้ผู้ใช้กดปุ่ม Sync เองถ้าต้องการข้อมูลล่าสุด
            }
        }, 30000);
    ...
    saveToLocalBackup(log) {
        ...
        try {
            ...
            if (backup.length > 200) backup.shift();

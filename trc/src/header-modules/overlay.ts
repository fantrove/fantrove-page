...
#${OVERLAY_ID} .spinner-svg { margin-bottom:12px;width:56px;height:56px;display:inline-flex; }
#${OVERLAY_ID} .spinner-svg svg { width:100%; height:100%;}
#${OVERLAY_ID} .spinner-svg-fg { stroke: #4285f4; stroke-width:5; stroke-linecap:round; stroke-dasharray:90 125; animation: instant-spinner-rotate 1s linear infinite; fill:none;}
#${OVERLAY_ID} .loading-message { font-size:1.06rem; color:#2196f3; text-align:center; margin-top:6px; font-weight:500;}
...
function buildOverlayElement(message: string, zIndex ? : number) {
...
function defaultMessageForLang(lang ? : string) {
  if (lang === 'th') return 'กำลังโหลดเนื้อหา...';
  return 'Loading content...';
}

export function showInstantLoadingOverlay(options: { lang ? : string;message ? : string;zIndex ? : number;autoHideAfterMs ? : number } = {}) {
  try {
    ensureStyles();

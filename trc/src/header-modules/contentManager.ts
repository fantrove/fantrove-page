// contentManager.ts (แปลงมาจาก JS)
// NOTE: ใช้ many `any` ในที่ซับซ้อนเพื่อให้แปลงได้รวดเร็ว — คุณสามารถค่อยๆ เพิ่มชนิดเข้มงวดขึ้นได้
export const contentManager: any = {
  _renderSession: 0,
  _abortController: null as AbortController | null,
  _virtualNodes: [] as HTMLElement[],
  _items: [] as any[],
  _renderedSet: new Set<number>(),
  _sentinelObserver: null as IntersectionObserver | null,
  _cleanupRender: null as (() => void) | null,
  _scheduleId: null as any,
  _isUnmounted: false,

  _elementPool: [] as HTMLElement[],
  _poolMax: 20,
  _poolCleanupTimer: null as any,
  _poolLastUsed: 0,

  _frameSamples: [] as number[],
  _avgFrameTime: 16,
  _baseBatch: 6,
  _minBatch: 3,
  _maxBatch: 18,
  ...
  _maxBatchLimit: 12,
  _deviceMemory: (navigator as any).deviceMemory || 4,
  _isSlowDevice: ((navigator as any).deviceMemory && (navigator as any).deviceMemory <= 2) || false,

  _learningWorker: null as Worker | null,
  _learningData: { views: {} as Record<string, number>, clicks: {} as Record<string, number> },
  _learningEnabled: true,
  _lastScores: {} as Record<string, number>,

  _SENTINEL_ID: 'headerv2-render-sentinel',
  _isRenderingNextBatch: false,
  _throttledScrollCheck: null as any,
  _pendingDOMUpdates: [] as any[],
  ...
  _releaseToPool(node: HTMLElement) {
    if (!node) return;
    try {
      node.innerHTML = '';
      node.className = '';
  ...
  _recordFrameSample(durationMs: number) {
  ...
  _getBatchLimit() {
    try {
      if (typeof window !== 'undefined' && (window as any)._headerV2_contentManagerConfig && (window as any)._headerV2_contentManagerConfig.maxBatchLimit != null) {
        const v = Number((window as any)._headerV2_contentManagerConfig.maxBatchLimit);
  ...
  _initLearningWorkerIfNeeded(itemsCount = 0) {
    ...
    try {
      ...
      this._learningWorker.onmessage = (e: MessageEvent) => {
        const { type, payload } = e.data || {};
  ...
  _recordEvent(kind: 'view' | 'click', id?: string) {
    if (!id) return;
    try {
  ...
  _getPriorityScoresFor(ids: string[]) {
      ...
      if (this._learningWorker) {
        ...
        const onmsg = (e: MessageEvent) => {
          if (e.data && e.data.type === 'scores') {
            clearTimeout(timer);
            this._learningWorker!.removeEventListener('message', onmsg as any);
            resolve(e.data.payload || fallback);
          }
        };
        this._learningWorker.addEventListener('message', onmsg as any);
  ...
  async clearContent() {
    ...
    try { (window as any)._headerV2_contentLoadingManager.hide(); } catch {}
    ...
    if (this._scheduleId) {
      try {
        if (typeof (window as any).cancelIdleCallback === 'function') (window as any).cancelIdleCallback(this._scheduleId);
  ...
  async renderContent(data: any[]) {
    if (!Array.isArray(data)) throw new Error('Content data should be array');
    const container = document.getElementById((window as any)._headerV2_contentLoadingManager.LOADING_CONTAINER_ID);
    if (!container) return;
    ...
    try {
      ...
      try { (window as any)._headerV2_contentLoadingManager.show({ behindSubNav }); } catch (e) {}
    } catch (e) {}
    ...
    const idList = items.map((it: any, idx: number) => (it && it.id) ? it.id : `__idx_${idx}`);
    ...
    const createSentinel = () => {
      let sentinel = document.getElementById(this._SENTINEL_ID) as HTMLElement | null;
      if (!sentinel) {
    ...
    const renderBatch = async (startIndex: number, batchSize: number) => {
      ...
      for (let i = startIndex; i < end; i++) {
        ...
        if (item && item.jsonFile && !item._fetched) {
          try {
            try { (window as any)._headerV2_contentLoadingManager.updateMessage('Loading...'); } catch {}
            const fetched = await (window as any)._headerV2_dataManager.fetchWithRetry(item.jsonFile, { cache: true }, 3).catch(err => { throw err; });
            if (Array.isArray(fetched)) {
              ...
              i = i - 1;
              continue;
            } else if (typeof fetched === 'object' && fetched !== null && Array.isArray((fetched as any).data)) {
              const arr = (fetched as any).data;
              item._fetched = true;
      ...
      if (frag.childNodes.length > 0) {
        ...
        requestAnimationFrame(() => {
          for (const node of appended) (node as HTMLElement).style.opacity = '1';
        });
      }
    ...
    const scheduleIdle = (fn: () => void) => {
      if ('requestIdleCallback' in window) (window as any).requestIdleCallback(fn, { timeout: 200 });
    ...
    if (renderedCount >= items.length) {
      try { (window as any)._headerV2_contentLoadingManager.hide(); } catch (e) {}
      return;
    }
    ...
    let sentinelDebounceTimer: any = null;
  ...
  async renderGroupItems(container: HTMLElement, group: any) {
    if (!group.categoryId && !group.items) throw new Error("Group ต้องระบุ categoryId หรือ items");

    if (group.categoryId) {
      const dm = (window as any)._headerV2_data_manager || (window as any)._headerV2_dataManager;
      const res = dm?.fetchCategoryGroup ? await dm.fetchCategoryGroup(group.categoryId) : await (window as any)._headerV2_dataManager.fetchCategoryGroup(group.categoryId);
      const { id, name, data, header } = res || {};
      if (header) {
  ...
  createGroupHeader(headerConfig: any) {
  ...
  createSimpleHeader(text: string, container: HTMLElement) {
  ...
  createHeaderComponents(container: HTMLElement, config: any, currentLang: string) {
  ...
  createHeaderTitle(config: any, currentLang: string) {
    const title = document.createElement('h2');
    title.className = 'group-header-text';
    if (typeof config.title === 'object') {
      Object.entries(config.title).forEach(([lang, text]) => {
        (title as any).dataset[`title${lang.toUpperCase()}`] = text;
      });
  ...
  createHeaderDescription(description: any, currentLang: string) {
    const desc = document.createElement('p');
    desc.className = 'group-header-description';
    if (typeof description === 'object') {
      Object.entries(description).forEach(([lang, text]) => {
        (desc as any).dataset[`desc${lang.toUpperCase()}`] = text;
      });
  ...
  addLanguageChangeListener(container: HTMLElement, config: any) {
    if (! (container as any)._langListenerBound) {
      window.addEventListener('languageChange', (event: any) => {
        const newLang = event.detail.language;
        this.updateHeaderLanguage(container, config, newLang);
      });
      (container as any)._langListenerBound = true;
    }
  },

  updateHeaderLanguage(container: HTMLElement, config: any, newLang: string) {
  ...
  async renderSingleItem(container: HTMLElement, item: any) {
    if (item.categoryId) {
  ...
  async createButton(config: any) {
    ...
    try {
      if (apiCode) {
        const db = await (window as any)._headerV2_data_manager?.loadApiDatabase?.() || await (window as any)._headerV2_dataManager.loadApiDatabase();
        function findApiNode(obj: any, code: string): any {
    ...
    button.addEventListener('click', async () => {
      ...
      try {
        await ((window as any).unifiedCopyToClipboard || (globalThis as any).unifiedCopyToClipboard).call(null, {
          text: finalContent,
          api: apiCode,
          type,
        ...
        (window as any)._headerV2_utils.showNotification('Copy failed', 'error');
      }
    });
  ...
  async createCard(cardConfig: any) {
    ...
    if (typeof cardConfig.title === 'object') {
      Object.entries(cardConfig.title).forEach(([langCode, text]) => {
        (titleDiv as any).dataset[`title${langCode.toUpperCase()}`] = text;
      });
    ...
    if (typeof cardConfig.description === 'object') {
      Object.entries(cardConfig.description).forEach(([langCode, text]) => {
        (descDiv as any).dataset[`desc${langCode.toUpperCase()}`] = text;
      });
  ...
  _animateOutAndRemove(node: HTMLElement, duration: number) {

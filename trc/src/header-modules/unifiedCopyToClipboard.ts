...
export async function unifiedCopyToClipboard(copyInfo: any = {}) {
  const lang = localStorage.getItem('selectedLang') || 'en';
  try {
    if (!copyInfo || !copyInfo.text) throw new Error('No content to copy');
    await navigator.clipboard.writeText(copyInfo.text);

    let notificationParams: any = { text: copyInfo.text, name: "", typeId: "emoji", lang };
    ...
    function findTypeIdAndName(obj: any, code: string, parentTypeId: string) {
      if (Array.isArray(obj)) {
    ...
    if (copyInfo.api) {
      ...
      if (node) {
        ...
        function getTypeFromParent(obj: any, targetNode: any, parentTypeId: string) {
          if (Array.isArray(obj)) {
            for (const item of obj) {
    ...
    if (typeof (window as any).showCopyNotification === "function") {
      (window as any).showCopyNotification(notificationParams);
    } else {
      (window as any)._headerV2_utils.showNotification(notificationParams.text, "success", { duration: 2200 });
    }

  } catch (error: any) {
    (window as any)._headerV2_utils.showNotification(error.message || 'Copy failed', 'error');
  }
}

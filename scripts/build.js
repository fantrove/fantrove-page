...
const CONFIG = {
  ...
  /**
   * Hidden directories to copy as-is to dist/ root (no processing).
   * These dirs start with '.' and are skipped by copyDir/findHtmlFiles,
   * so they must be explicitly listed here to be included in the build output.
   *
   * Examples:
   *  - .well-known → for domain verification (Bluesky atproto-did, etc.)
   *  - .github     → GitHub-specific config (FUNDING.yml, etc.)
   */
...
async function build() {
  ...
  if (!DRY_RUN) {
    ...
    // Copy pass-through hidden directories (e.g. .well-known) as-is

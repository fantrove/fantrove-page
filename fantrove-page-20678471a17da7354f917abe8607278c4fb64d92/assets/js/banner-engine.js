/**
 * banner-engine.js  v5.0.0
 *
 * 2 rendering modes:
 *   'builder' — builds HTML from JSON config, renders inside Shadow DOM
 *   'html'    — mounts customHtml[lang] + customCss[lang] inside Shadow DOM
 *
 * WHY Shadow DOM:
 *   The banner must look identical on any host page regardless of that page's
 *   CSS. Shadow DOM creates a style boundary — host page styles cannot bleed
 *   in, and banner styles cannot leak out. This guarantees preview == live.
 *
 * Multi-language: reads selectedLang from localStorage.
 * Cache: 60s with stale-while-revalidate background refresh.
 */
(function (global) {
  'use strict';
  ...
  function _getCached(slug) {
    ...
    _fetchAndRender(slug, false); // background refresh
    return entry.data;
  }
  ...
  // i18n substitution — works on Shadow DOM root or regular element
  function _applyI18n(root, translations, lang) {
  ...
  // ── Shadow DOM helpers ─────────────────────────────────────────────────────
  // WHY: Reuse the same shadow root across re-renders to avoid flicker.
  function _getShadow(mountEl) {
    if (mountEl._beShadow) return mountEl._beShadow;
    // Check if shadow root already attached (e.g. SSR hydration)
    if (mountEl.shadowRoot) { mountEl._beShadow = mountEl.shadowRoot; return mountEl.shadowRoot; }
    var shadow = mountEl.attachShadow({ mode: 'open' });
    mountEl._beShadow = shadow;
    return shadow;
  }

  // ── BASE CSS (builder mode) ────────────────────────────────────────────────
  // Mirrored from bannerTemplate.ts BASE_CSS.
  // Injected inside Shadow DOM so it is fully isolated from host page.
  var _BASE_CSS = [
    '*,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}',
    ':host{display:block;}',
    '.be-banner{position:relative;overflow:hidden;border-radius:16px;padding:28px 24px;display:flex;flex-direction:column;gap:14px;background:linear-gradient(135deg,#13b47f 0%,#0eb0d5 100%);color:#fff;font-family:Inter,system-ui,-apple-system,sans-serif;box-shadow:0 4px 24px rgba(0,0,0,.12);}',
  ...
  // ── Builder mode: build inner HTML from JSON config ────────────────────────
  ...
  function _startCountdown(shadow, slug) {
    var cdEl = shadow.querySelector('.be-countdown');
    if (!cdEl) return;
    ...
    function tick() {
      ...
      shadow.querySelectorAll('[data-cd-unit]').forEach(function(el){
  ...
  function _startSlider(shadow, slug) {
    var sliderEl = shadow.querySelector('.be-slider');
    if (!sliderEl) return;
  ...
  // ── JS Triggers ───────────────────────────────────────────────────────────
  // Triggers receive the Shadow DOM wrapper element (or host element in html mode).
  var JS_TRIGGERS = {
    confetti: function(el) {
      el.style.position = 'relative';
      for (var i=0;i<30;i++){(function(i){
        var d=document.createElement('span');
        d.style.cssText='position:absolute;width:6px;height:6px;border-radius:50%;pointer-events:none;background:'+['#13b47f','#0eb0d5','#fff','#ffd166'][i%4]+';left:'+Math.floor(Math.random()*100)+'%;top:'+Math.floor(Math.random()*100)+'%;opacity:.95;z-index:1;';
        el.appendChild(d);
        setTimeout(function(){d.style.transition='transform .9s ease,opacity .9s ease';d.style.transform='translateY(120px) scale(.6)';d.style.opacity='0';setTimeout(function(){d.remove();},1000);},2);
      })(i);}
    },
    shake: function(el) {
      var style = document.createElement('style');
      style.textContent = '@keyframes be-shake{0%,100%{transform:translateX(0)}20%,60%{transform:translateX(-6px)}40%{transform:translateX(6px)}}';
      document.head.appendChild(style);
      el.style.animation = 'be-shake .4s ease';
      el.addEventListener('mouseover', function(){
        el.style.animation='none';
        requestAnimationFrame(function(){el.style.animation='be-shake .4s ease';});
      });
    },
    pulse: function(el) {
      var style = document.createElement('style');
      style.textContent = '@keyframes be-pulse{0%,100%{box-shadow:0 0 0 0 rgba(19,180,127,.2)}50%{box-shadow:0 0 26px 6px rgba(19,180,127,.04)}}';
      document.head.appendChild(style);
    ...
    scroll_reveal: function(el) {
      ...
      var io = new IntersectionObserver(function(e){
        if (e[0].isIntersecting){el.style.opacity='1';el.style.transform='translateY(0)';io.disconnect();}
      },{threshold:0.15});
      io.observe(el);
    },
    bounce: function(el) {
      var style = document.createElement('style');
      style.textContent = '@keyframes be-bounce{0%,100%{transform:translateY(0)}30%{transform:translateY(-6px)}50%{transform:translateY(0)}}';
      document.head.appendChild(style);
      el.style.animation = 'be-bounce .6s cubic-bezier(.36,.07,.19,.97)';
    },
    glow: function(el) {
      var style = document.createElement('style');
      style.textContent = '@keyframes be-glow{from{box-shadow:0 0 8px rgba(19,180,127,.3)}to{box-shadow:0 0 18px rgba(14,176,213,.25)}}';
      document.head.appendChild(style);
  ...
  function _render(mountEl, data) {
    var lang   = _getLang();
    var slug   = data.slug;
    var shadow = _getShadow(mountEl);

    mountEl.setAttribute('data-banner-mount', slug);

    if (data.editorMode === 'html') {
      // ── HTML mode: user's HTML + CSS inside Shadow DOM ──────────────────
      var rawHtml = (data.customHtml && (data.customHtml[lang] || data.customHtml['en'])) || '';
      var rawCss  = (data.customCss  && (data.customCss[lang]  || data.customCss['en']))  || data.bannerStyles || '';

      shadow.innerHTML = '<style>' + rawCss + '</style>' + _sanitizeHtml(rawHtml);

      if (data.translations) _applyI18n(shadow, data.translations, lang);
      _startCountdown(shadow, slug);
      _startSlider(shadow, slug);

      // Trigger attaches to first child element in html mode
      var firstEl = shadow.querySelector('*:not(style)');
      if (firstEl && data.jsTrigger && JS_TRIGGERS[data.jsTrigger]) {
        try { JS_TRIGGERS[data.jsTrigger](firstEl); } catch(e) { console.warn('[banner-engine] trigger:', e); }
      }

    } else {
      // ── Builder mode: .be-banner wrapper inside Shadow DOM ──────────────
      var userCss = String(data.bannerStyles || '').replace(/<\/style>/gi,'');
      var inner   = _buildInnerHtml(data, lang);

      shadow.innerHTML = '<style>' + _BASE_CSS + userCss + '</style>'
        + '<div class="be-banner banner-custom">' + inner + '</div>';

      if (data.translations) _applyI18n(shadow, data.translations, lang);
      _startCountdown(shadow, slug);
      _startSlider(shadow, slug);

      var wrapper = shadow.querySelector('.be-banner');
      if (wrapper && data.jsTrigger && JS_TRIGGERS[data.jsTrigger]) {
        try { JS_TRIGGERS[data.jsTrigger](wrapper); } catch(e) { console.warn('[banner-engine] trigger:', e); }
      }
    }
  }
